...
class RPZPack(object):
    ...
    def _extract_file(self, name, target):
        if self.tar is not None:
            ...
            self.tar.extract(member, path=str(Path.cwd() / target.parent), filter='data')
        else:
            member = copy.copy(self.zip.getinfo(name))
            member.filename = target.name
    ...
    def with_config(self):
        ...
        """
        fd, tmp_str = tempfile.mkstemp(prefix='reprounzip_')
        os.close(fd)
        tmp = Path(tmp_str)
    ...
    def get_data(self, path):
        ...
        data_path = PurePosixPath('DATA') / path.relative_to('/')
        return copy.copy(self.data.getmember(str(data_path)))
    ...
    def extract_data(self, root, members):
        ...
        for member in members:
            ...
            if not member_path.is_relative_to(abs_root):
                raise ValueError("Invalid path in data tar")

        self.data.extractall(str(root), members, filter='data')
    ...
    def copy_data_tar(self, target):
        ...
        if self.tar is not None:
            if self.version == 1:
                shutil.copy2(str(self.pack), str(target))
            elif self.version == 2:
...
def enable_usage_report(enable):
    stderr.write("Usage reporting is not available.\n")
...
def record_usage_package(runs, packages, other_files, inputs_outputs, pack_id=None):
    pass
